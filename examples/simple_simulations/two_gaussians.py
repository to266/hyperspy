u"""Creates a 2D hyperspectrum consisting of two gaussians and plots it.
...
gs1.centre.map[u'values'][:] = 256 + (np.random.random((32, 32)) - 0.5) * 10
gs1.centre.map[u'is_set'][:] = True

# Make the area vary between 0 and 10000
gs1.A.map[u'values'][:] = 10000 * np.random.random((32, 32))
gs1.A.map[u'is_set'][:] = True
...
gs2.centre.map[u'values'][:] = 768 + (np.random.random((32, 32)) - 0.5) * 20
gs2.centre.map[u'is_set'][:] = True

# Make the area vary between 0 and 20000
gs2.A.map[u'values'][:] = 20000 * np.random.random((32, 32))
gs2.A.map[u'is_set'][:] = True

# Create the dataset
s_model = m.as_signal()

# Add noise
s_model.set_signal_origin(u"simulation")
