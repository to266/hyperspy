...
class TestLoadingNewSavedMetadata(object):

    def setUp(self):
        self.s = load(os.path.join(
            my_path,
            u"hdf5_files",
            u"with_lists_etc.hdf5"))
...
class TestSavingMetadataContainers(object):
    ...
    def test_save_unicode(self):
        s = self.s
        s.metadata.set_item(u'test', [u'a', u'b', u'\u6f22\u5b57'])
        s.save(u'tmp.hdf5', overwrite=True)
        l = load(u'tmp.hdf5')
    ...
    def test_save_long_list(self):
        s = self.s
        s.metadata.set_item(u'long_list', range(10000))
        s.save(u'tmp.hdf5', overwrite=True)

    def test_numpy_only_inner_lists(self):
        s = self.s
        s.metadata.set_item(u'test', [[1., 2], (u'3', 4)])
        s.save(u'tmp.hdf5', overwrite=True)
        l = load(u'tmp.hdf5')
    ...
    def test_numpy_general_type(self):
        ...
        nt.assert_is_instance(l.metadata.test[1][0], unicode)
        nt.assert_is_instance(l.metadata.test[1][1], unicode)

    def test_general_type_not_working(self):
        s = self.s
        s.metadata.set_item(u'test', (Signal([1]), 0.1, u'test_string'))
        s.save(u'tmp.hdf5', overwrite=True)
        l = load(u'tmp.hdf5')
...
class TestLoadingOOMReadOnly(object):
    ...
    def setUp(self):
        ...
        del s
        f = h5py.File(u'tmp.hdf5', model=u'r+')
        s = f[u'Experiments/__unnamed__']
        del s[u'data']
        s.create_dataset(
            u'data',
            shape=self.shape,
    ...
    def test_oom_loading(self):
        s = load(u'tmp.hdf5', load_to_memory=False)
