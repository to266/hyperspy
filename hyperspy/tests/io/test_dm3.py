...
def test_content():
    for subfolder in data_dict:
        for key, data in data_dict[subfolder].items():
            if subfolder == u'dm3_1D_data':
                dat = np.arange(1, 3)
            elif subfolder == u'dm3_2D_data':
                dat = np.arange(1, 5).reshape(2, 2)
            elif subfolder == u'dm3_3D_data':
